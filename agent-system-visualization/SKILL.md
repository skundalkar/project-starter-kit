...
- Give artifact labels collision-free vertical tracks within each artifact row. Size row height from its track count, keep a readable text gap, draw handoff ribbons behind artifact dots/labels, and give labels an opaque text halo when connectors pass nearby. Browser-check rendered label bounding boxes for overlap.
